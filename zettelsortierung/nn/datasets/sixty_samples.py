...
class SixtySamples(BaseDocumentDataset):
    def __init__(self, train: bool):
        full_df = self._load_df()
    ...
    def _load_df(self) -> pd.DataFrame:
        ...
        path = f"{root}/data/interim/class_112x60.parquet"
        return pd.read_parquet(path)


def main():
    ss = SixtySamples(train=False)
    print(ss.get_classes())
    print(ss[2])
